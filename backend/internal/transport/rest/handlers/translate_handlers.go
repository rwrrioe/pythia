...
func (h *TranslateHandler) isErr(ok bool, err error, req models.AnalyzeRequest, stage string) bool {
	if ok != true {
...
func (h *TranslateHandler) Translate(c *gin.Context) {
	...
	go func() {
		...
		if h.isErr(ok, err, req, "translate") {
			return
		}

		words, err := h.transl.FindUnknownWords(c, task, req)
		if h.isErr(true, err, req, "translate") {
			return
		}
	...
	c.JSON(http.StatusAccepted, gin.H{"task_id": req.TaskID, "stage": "translate"})
}

// WriteExamples post /api/translate/examples
func (h *TranslateHandler) WriteExamples(c *gin.Context) {
	...
	go func() {
		...
		task, ok, err := h.storage.Get(c, req.TaskID)
		if h.isErr(ok, err, req, "writing examples") {
			return
		}

		examples, err := h.transl.WriteExamples(c, task, req)
		if h.isErr(true, err, req, "writing examples") {
			return
		}
	...
	c.JSON(http.StatusAccepted, gin.H{"task_id": req.TaskID, "stage": "writing examples"})
}
