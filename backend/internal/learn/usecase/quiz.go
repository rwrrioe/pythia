...
import (
	...
	"github.com/rwrrioe/pythia/backend/internal/learn/domain"
)
...
func (s *LearnService) QuizTest(ctx context.Context, words []domain.Word) []domain.QuizQuestion {
	var test []domain.QuizQuestion

	for _, v := range words {
		opts := pickOptions(words, v.Word, s.Optcount)
		questionDTO := domain.QuizQuestion{
			Answer:   v.Word,
...
func pickOptions(words []domain.Word, correct string, optcount int) []string {
	var pool []string
